...
const ORDER_URL = process.env.NEXT_PUBLIC_ORDER_URL || "http://localhost:4001";
...
export default function Page() {
  ...
  useEffect(() => {
    const loadCart = async () => {
      ...
      if (!token) {
        ...
        try {
          const savedUserCart = window.localStorage.getItem(`cart:${userKey}`);
          if (savedUserCart) {
      ...
      try {
        ...
        if (data.success && data.cart?.items) {
          ...
          window.localStorage.setItem(`cart:${userKey}`, JSON.stringify(items));
        }
      } catch (err) {
  ...
  // Debounced save cart to backend whenever it changes
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem(`cart:${userKey}`, serialized);
    } catch {}
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex justify-between items-center">
                    <button 
                      ...
                      className="relative z-10 inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    >
                      Add to cart
                    </button>
      ...
      <footer className="bg-white">
        <div className="max-w-7xl mx-auto py-12 px-4 overflow-hidden sm:px-6 lg:px-8">
          ...
          <p className="mt-8 text-center text-base text-gray-400">
            &copy; 2026 MicroServices Project POC.
          </p>
        </div>
      </footer>
    </div>
